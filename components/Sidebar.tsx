...
type NavItem = {
  ...
  flag?: "allotments";
  pageKey?: string;
  // Requires the role AND the tick — no automatic grant for admins.
  strictPageKey?: string;
};
...
const navItems: NavItem[] = [
  ...
  },
  {
    ...
    href: "/recoveries",
    roles: ["admin", "ops_manager", "hub_incharge"],
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12a9 9 0 1 0 9-9"/><polyline points="3 3 3 12 12 12"/></svg>,
  },
  {
    label: "Hubs",
    href: "/hubs",
  ...
  },
  {
    ...
    href: "/finance",
    roles: ["admin"],
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 1v22"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/><circle cx="12" cy="12" r="10"/></svg>,
  },
  {
    label: "Recon",
    href: "/recon",
    roles: ["admin"],
    strictPageKey: "recon",
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 8V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v3"/><path d="M3 16v3a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-3"/><polyline points="7 11 9 13 13 9"/><line x1="16" y1="12" x2="20" y2="12"/></svg>,
  },
  {
    label: "My Portfolio",
    href: "/portfolio",
...
export default function Sidebar({ role, name, canViewAllotments, appPages, onClose }: Props) {
  ...
  const visibleItems = navItems.filter((item) => {
    ...
    // A strict page is never granted by role alone — it needs the role AND the
    // tick, which is why Recon does not appear for every admin.
    if (item.strictPageKey) {
      return item.roles.includes(role) && (appPages ?? []).includes(item.strictPageKey);
    }
    // Admins see every other App Pages section; everyone else needs it enabled.
    if (item.pageKey) {
