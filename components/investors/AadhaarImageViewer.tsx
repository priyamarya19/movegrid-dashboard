  ...
  return (
    <>
      <button
        ...
        className="text-[#6C5CE7] hover:text-[#8b7dff] transition-colors"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setOpen(false)}>
          ...
          <div className="relative max-w-2xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-white text-sm font-medium">Aadhaar</span>
              <button onClick={() => setOpen(false)} className="text-gray-300 hover:text-white">✕</button>
            </div>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={src} alt="Aadhaar" className="w-full rounded-xl border border-white/10 bg-[#12121A]" />
          </div>
        </div>
      )}
    </>
