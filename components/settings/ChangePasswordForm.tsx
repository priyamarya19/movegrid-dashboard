...
export default function ChangePasswordForm() {
  ...
  const inputClass = "w-full bg-base border border-default rounded-xl px-4 py-2.5 pr-10 text-primary text-sm placeholder-faint focus:outline-none focus:border-accent-success transition-colors";

  return (
    <div className="bg-surface border border-default rounded-xl p-5">
      <div className="flex items-center gap-2.5 mb-5">
        <div className="w-8 h-8 bg-accent-purple/13 rounded-lg flex items-center justify-center">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--accent-purple)" strokeWidth="2">
            <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>
          </svg>
        </div>
        <div>
          <h2 className="text-primary font-semibold">Change Password</h2>
          <p className="text-muted text-xs">Update your login password</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="sm:col-span-3 flex items-center gap-3 pt-1">
          <button
            type="submit"
            disabled={loading}
            className="px-5 py-2.5 rounded-xl bg-accent-purple hover:bg-accent-purple text-primary text-sm font-semibold disabled:opacity-60 transition-colors"
          >
            {loading ? "Updating..." : "Update Password"}
          </button>
          {error && (
            <p className="text-accent-danger-alt-text text-sm">{error}</p>
          )}
          {success && (
            <p className="text-accent-success-text text-sm flex items-center gap-1.5">
