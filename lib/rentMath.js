...
function generateWeeks({ startISO, cutoffISO, startWeekNo = 1 }) {
  ...
  while (daysBetween(ps, cutoffISO) > 0) {
    ...
    weeks.push({
      ...
      periodStart: ps,
      periodEnd: pe,
      // Rent in advance: due the day before the cycle — except the tenancy's very
      // first week, which is paid on the allotment day itself (there is no "day
      // before" a tenancy that hasn't started; same for a re-allotment's week 1).
      dueDate: weekNo === 1 ? ps : addDaysISO(ps, -1),
    });
    lastPeriodEnd = pe;
    ps = addDaysISO(ps, 7);
    weekNo += 1;
