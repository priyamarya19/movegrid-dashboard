  ...
  return (
    <DashboardLayout allowedRoles={["admin", "ops_manager", "hub_incharge"]}>
      <div className="space-y-6">
        <div>
          <h1 className="text-white text-2xl font-bold">Operations Forms</h1>
          <p className="text-[#666] text-sm mt-1">Fill these forms to log entries directly into the system</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {visible.map((form) => (
            <Link key={form.href} href={form.href} className="group bg-[#12121A] border border-[#1e1e2e] rounded-xl p-6 hover:border-[#333] hover:bg-[#16161f] transition-all block">
              <div className={`w-11 h-11 rounded-xl ${form.bg} flex items-center justify-center mb-4`} style={{ color: form.color }}>
                {form.icon}
              </div>
              <h2 className="text-white font-semibold text-base mb-1.5 group-hover:text-white">{form.title}</h2>
              <p className="text-[#555] text-sm leading-relaxed">{form.description}</p>
